...
ASTHMA_wald <- function(x1, x2, p, n.B, show.plot = T, fmt = '%1.3f') {
  #' Author: CWM
  #' Purpose: Wald paper function
  #' Date: 10.21.2019
  #' Arguments:
  ...
  # plot
  if(show.plot) {
    pp <- ggplot(data.frame(x = x1, y = x2)) +
      geom_density(aes(x = x), fill = "pink", alpha = 0.5) +
      geom_density(aes(x = y), fill = "lightgreen", alpha = 0.5)
    print(pp)
  }
...
```{r block2}
norm.pop <- c(rnorm(500, mean = 0, sd = 1), rnorm(500, 5, 1))

cauchy.pop <- c(rcauchy(500, location = 0, scale = 0.5), rnorm(500, 10, 1))

p <- c(0.25, 0.50, 0.75)

n.B <- 500

ASTHMA_wald(norm.pop, cauchy.pop, p, n.B, show.plot = T)
```